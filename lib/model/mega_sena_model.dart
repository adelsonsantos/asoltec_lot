...
class MegaSenaModel {
  List<Megasena> megasena = [];
  String date = '';
  String error = '';

  MegaSenaModel({required this.megasena});

  MegaSenaModel.withError(errorMessage) {
    error = errorMessage;
  }

  MegaSenaModel.fromJsonn(List<dynamic> json) {
    megasena = [];
    for (var v in json) {
      megasena.add(Megasena.fromJson(v));
    }
  }
}
...
class Megasena {
  ...
  Megasena.fromJson(Map<String, dynamic> json) {
  ...
  Megasena.withError(errorMessage) {
    error = errorMessage;
  }
}
